...
window.addEventListener('load', function load(event) {
  let hour = d.getHours();
  if(hour < 12) {
    greetElement.innerHTML = greetings[0];
  } else if(hour > 12 && hour < 17) {
    greetElement.innerHTML = greetings[1];
  } else {
    greetElement.innerHTML = greetings[2];
  }
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(getWeather);
  } else { 
    locElement.innerHTML = "Weather Unavalible";
  }
})
...
function getWeather(position) {
  ...
  let url = "http://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + lng + "&units=imperial&appid=147e9f280d3581fc8be5fd31a0c09f35";
  
  fetch(url)
  .then(res => res.json())
  .then((out) => {
    var temp = Math.round(out.main.temp);
    console.log(temp);
    locElement.innerHTML = temp + "&deg";
    dateElement.innerHTML = days[d.getDay()] + ", " + months[d.getMonth()] + " " + d.getDay();
  })
  .catch(err => { throw err });
}
